...
for dataset_name in ["math500"]:
    ...
    OUTPUT_PATH = f"src/eval/math-500/eval_{dataset_name}_{file_base}_greedy_output.json"
    PROMPT_PATH = f"src/eval/math-500/eval_{dataset_name}_for_bench.json"
    print("Output_Paht: ", OUTPUT_PATH)
    ...
    for i in tqdm(range(start_idx, len(messages), BSZ), desc="Processing batches"):
        ...
        try:
            # batch_messages에서 multimodal이 하나라도 있으면 multimodal 처리
            has_mm = any(msg[0]['content'][0]['type'] in ['image', 'video'] for msg in batch_messages)
            if has_mm:
